...
betweenCharTok cl cr = between l r
  where l = charTok cl
        r = charTok cr
...
-- True
hex :: Parser Char
hex = replicateA 4 (satisfy (isHexDigit)) >>= f . readHex
  where f (Full x) = pure $ toEnum x
        f _        = failed
...
-- True
eof :: Parser ()
eof = P $ \s -> case s of Nil -> Result Nil ()
...
-- satisfyAll fs = satisfy $ \c -> and [f c | f <- fs]
satisfyAll fs = satisfy $ \c -> and (fs <*> pure c)
...
satisfyAny fs = satisfy $ \c -> or (fs <*> pure c)
