...
untilM :: Monad m => (a -> m Bool) -- ^ The predicate to satisfy to stop running the action.
...
convertInteractive = putStr "Enter a phrase or whatever, then hit enter... >"
                      >- getLine
...
reverseInteractive = putStr "Give me an input filepath please !! >"
                     >- getLine
                    >>= \inp -> putStr "Give me an output filepath now of course, duh >"
                                 >- getLine
                                >>= \out -> readFile inp
                                            >>= writeFile out . reverse
...
encodeInteractive = putStr "Give me a string to url encode, c'mon >"
                     >- getLine
                    >>= putStrLn . (encode =<<)
