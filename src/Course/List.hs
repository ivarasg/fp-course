...
-- map f = foldRight (\el acc -> (f el):.acc) Nil
...
filter f = foldRight satisfies Nil
  where satisfies el acc = if   f el
                           then el:.acc
                           else acc
...
-- find f = foldLeft helper Empty
--   where helper acc el = if   f el
--                         then acc <+> (Full el)
--                         else acc <+> Empty
...
lengthGT4 = fst . helper 1
  where helper _ Nil     = (False, 0)
        helper n (_:.xs) = if n > 4
                            then (True, n)
                            else helper (n + 1) xs
