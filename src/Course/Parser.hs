...
mapParser f (P p) = P $ p'
  where p' s = case p s of
                (Result input v )  -> Result input (f v)
                (ErrorResult perr) -> ErrorResult perr
...
bindParser f p = P $ \s -> case parse p s of
                                (ErrorResult perr) -> ErrorResult perr
                                (Result s' x)      -> parse (f x) s'
...
pa >>> pb = pa `flbindParser` const pb
...
-- satisfy predicate = P g
--   where g Nil           = ErrorResult UnexpectedEof
--         g (x:.xs)
--           | predicate x = Result xs x
--           | otherwise   = ErrorResult $ UnexpectedChar x
satisfy cond = character --handles EOF
               >>= \c -> if cond c
                          then pure c
                          else failedWith $ UnexpectedChar c
...
-- True
natural :: Parser Int
natural = list1 digit >>= \i -> case read i of
                                  Full i' -> pure i'
                                  _       -> failed
...
personParser = Person <$> ageParser' <*> firstNameParser' <*> surnameParser' <*>  smokerParser' <*> phoneParser
  where ageParser'       = ageParser <* spaces1
        firstNameParser' = firstNameParser <* spaces1
        surnameParser'   = surnameParser <* spaces1
        smokerParser'    = smokerParser <* spaces1
