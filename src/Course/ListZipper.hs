...
instance Functor ListZipper where
  ...
  g <$> ListZipper l f r = ListZipper (map g l) (g f) (map g r)
...
withFocus g (ListZipper l f r) = ListZipper l (g f) r
...
setFocus f (ListZipper l _ r) = ListZipper l f r
...
findLeft p (ListZipper l f r) = mlz
  where mlz = case snd rs of
                Nil     -> IsNotZ
                (x:.xs) -> IsZ $ ListZipper xs x (fst rs ++ f:.r)
        rs  = break p l
...
findRight p (ListZipper l f r) = mlz
  where mlz = case snd rs of
                Nil     -> IsNotZ
                (x:.xs) -> IsZ $ ListZipper ((reverse . fst) rs ++ f:.l) x xs
        rs  = break p r
...
moveLeftLoop (ListZipper Nil f r)     = let f':.l' = reverse (f:.r)
                                        in  ListZipper l' f' Nil
moveLeftLoop (ListZipper (l:.ls) f r) = ListZipper ls l (f:.r)
...
moveRightLoop (ListZipper l f Nil)     = let f':.r' = reverse (f:.l)
                                         in  ListZipper Nil f' r'
moveRightLoop (ListZipper l f (r:.rs)) = ListZipper (f:.l) r rs
...
moveLeft (ListZipper (l:.ls) f r) = IsZ $ ListZipper ls l (f:.r)
...
moveRight (ListZipper l f (r:.rs)) = IsZ $ ListZipper (f:.l) r rs
...
swapLeft (ListZipper (l:.ls) f r) = IsZ $ ListZipper (f:.ls) l r
...
swapRight (ListZipper l f (r:.rs)) = IsZ $ ListZipper l r (f:.rs)
...
moveLeftN n lz
  | n == 0    = IsZ lz
  | n <  0    = moveRightN (-n) lz
  | otherwise = (repeat' moveLeftN n) . moveLeft $ lz

repeat' :: (Int -> ListZipper a -> MaybeListZipper a) -> Int -> MaybeListZipper a -> MaybeListZipper a
repeat' _ _ IsNotZ   = IsNotZ
repeat' f n (IsZ lz) = f (n - 1) lz
...
moveRightN n lz
  | n == 0    = IsZ lz
  | n <  0    = moveLeftN (-n) lz
  | otherwise = (repeat' moveRightN n) . moveRight $ lz
...
moveLeftN' n lz
  | n <  0    = moveRightN' (-n) lz
  | otherwise = eval (moveN' moveLeft n lz) 0
  -- error "todo: Course.ListZipper#moveLeftN'"

moveN' :: (ListZipper a -> MaybeListZipper a) -> Int -> ListZipper a -> State Int (Either Int (ListZipper a))
moveN' _ 0 lz  = return $ Right lz
moveN' f n lz  = get
                 >>= \s -> case f lz of
                            IsNotZ    -> return $ Left s
                            (IsZ lz') -> put (s + 1) >> moveN' f (n - 1) lz'
...
moveRightN' n lz
  | n <  0    = moveLeftN' (-n) lz
  | otherwise = eval (moveN' moveRight n lz) 0
  -- error "todo: Course.ListZipper#moveRightN'"
...
nth n lz@(ListZipper l _ _) = moveRightN (n - ll) lz
    where ll = length l
...
end (ListZipper l f (r:.rs)) = end lz'
  where lz' = ListZipper (f:.l) r rs
...
start (ListZipper (l:.ls) f r) = start lz'
  where lz' = ListZipper ls l (f:.r)
...
insertPushLeft x (ListZipper l f r) = ListZipper (f:.l) x r
...
insertPushRight x (ListZipper l f r) = ListZipper l x (f:.r)
...
-- /Tip:/ Use @List#repeat@.
  pure x = let lr = repeat x in ListZipper lr x lr
    -- error "todo: Course.ListZipper pure#instance ListZipper"
-- /Tip:/ Use `zipWith`
  ListZipper lf f rf <*> ListZipper lx x rx = ListZipper l' (f x) r'
    where l' = zipWith ($) lf lx
          r' = zipWith ($) rf rx
    -- error "todo: Course.ListZipper (<*>)#instance ListZipper"
...
instance Extend ListZipper where
  f <<= lz = ListZipper l' (f lz) r'
    where l' = f <$> unfoldr ((twice <$>) . toOptional . moveLeft ) lz
          r' = f <$> unfoldr ((twice <$>) . toOptional . moveRight) lz
    -- error "todo: Course.ListZipper (<<=)#instance ListZipper"
...
instance Traversable ListZipper where
  traverse g (ListZipper l f r) = lift3 ListZipper (traverse g l) (g f) (traverse g r)
